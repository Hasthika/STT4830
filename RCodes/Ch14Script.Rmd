...
obs_exp_group_cou_red <- obs_exp %>%
   group_by(xlevels) %>%
  summarize(ni = n(), Pjhat = mean(fits), Obs_cou_red = sum(yvals), Exp_cou_red = ni*Pjhat)
...
obs_exp_group_cou_not_red <- obs_exp %>%
   group_by(xlevels) %>%
  summarize(ni = n(), Pjhat = mean(fits), Obs_cou_not_red = 200-sum(yvals), Exp_cou_not_red = ni*(1-Pjhat))
